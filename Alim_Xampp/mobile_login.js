    ...
    request.done(function(json) {
        ...
        if ( typeof json === "object" && json.list.length > 0) {
            $(json.list).each(function() {
                ...
                htmls += '<a onclick="classClick(\'' + this.class_key + '\',\'' + this.grade + '학년';
                if (this.ban == 0)
                    htmls += "전담 ";
                else
                    htmls += this.ban + "반 ";

                htmls += this.name + '선생님\',\'' + this.id + '\',\'' + this.day + '\')" class="list-group-item">' + this.grade + '학년' + this.ban + '반<br>' + this.name + '선생님' + '</a>';

            });
            $('#class_list').html(htmls);
        } else {
            htmls = '<a href="#"  class="list-group-item" value="' + '0' + '">' + '검색결과가 없습니다. <br/> <br/> ' + school + '에는 스피드알림장을 쓰는 학급이 없습니다.' + '</a>';
            $('#class_list').html(htmls);
        }
        htmls = "";
        isLoading = false;
...
function toNext() {
    ...
    if (class_key == input_key) {
        ...
        localStorage.setItem("ID", class_id);
        localStorage.setItem("KEY", class_key);
        $('<form action="mobile_today.php" method="POST">' + '<input type="hidden" name="platform" value=1>' + '<input type="hidden" name="post_id" value="' + class_id + '">' + '<input type="hidden" name="post_key" value="' + class_key + '">' + '</form>').submit();
        return;
    } else {
        // Android.showToast2();
        alert("학급키가 맞지 않아요.\선생님께 문의해보세요.")
        return;
    }
}
